...
p = subprocess.run(
    ["uv", "run", "pytest", "packages/database/tests", "-v", "--tb=short"],
    text=True,
    capture_output=True,
    env=env,
)
...
a = subprocess.run(
    ["uv", "run", "alembic", "-c", "packages/database/alembic.ini", "check"],
    text=True,
    capture_output=True,
)
...
d = subprocess.run(
    ["git", "grep", "apps.backend.worker", "packages/database/"], text=True, capture_output=True
)
