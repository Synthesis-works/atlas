...
def verify():
    ...
    with SessionLocal() as session:
        ...
        strat = EvaluationStrategy(
            name=f"test_exact_match_{uuid.uuid4().hex[:8]}", type="exact_match"
        )
        session.add(strat)
        session.commit()
    ...
    with SessionLocal() as session:
        ...
        print(
            f"✅ CapabilityProfile stored (version={profile.profile_version}, overall_score={profile.overall_score})"
        )
        print(
            f"✅ Score Explanation includes raw_measurements and weights: {'exact_match' in str(profile.score_explanation)}"
        )
