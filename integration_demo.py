...
from services.execution_service.app.commands.run import CreateRunCommand, ValidateRunCommand, CancelRunCommand
...
from services.execution_service.app.scheduler.policies import CompositePolicy, GlobalConcurrencyPolicy
from services.execution_service.app.recovery.manager import RecoveryManager
from services.execution_service.app.recovery.policies import CompositeRecoveryPolicy, RetryPolicy, TimeoutPolicy
...
def run_integration_demo():
    ...
    recovery_manager = RecoveryManager(db_session, publisher, unhealthy_threshold_seconds=2, offline_threshold_seconds=5)
    ...
    # 1. Register Worker
    worker1_id = worker_ctrl.execute_register_worker(RegisterWorkerCommand(
        adapter_id=uuid.uuid4(), name="Worker-1", version="1.0", hostname="host-1", platform="linux", region="us-east", hardware_info={}, capabilities={}
    ))
    print(f"[*] Worker-1 Registered.")
    
    # 2. Create & Validate Run
    run1_id = run_ctrl.execute_create_run(CreateRunCommand(
    ...
    print(f"[*] Run-1 Created and Validated (QUEUED).")
    
    # 3. Scheduler ticks
    scheduler.tick()
    print(f"[*] Scheduler ticked. Task scheduled to Worker-1.")
    
    # 4. Worker executes
    task = db_session.query(AtlasTask).filter_by(atlas_run_id=run1_id).one()
    task_ctrl.execute_complete_task(CompleteTaskCommand(worker_id=worker1_id, task_id=task.id, raw_output="success"))
    ...
    worker2_id = worker_ctrl.execute_register_worker(RegisterWorkerCommand(
        adapter_id=uuid.uuid4(), name="Worker-2", version="1.0", hostname="host-2", platform="linux", region="us-east", hardware_info={}, capabilities={}
    ))
    ...
    print(f"[*] Task scheduled to Worker-2.")
    
    # Simulate Worker 2 dying
    print(f"[*] Simulating Worker-2 crash (lease expired)...")
    ...
    print(f"[*] Recovery Manager detected failure. TaskController requeued task.")
    
    scheduler.tick()
    task2 = db_session.query(AtlasTask).filter_by(atlas_run_id=run2_id).one()
    print(f"[*] Scheduler reassigned task to Worker-1.")
    
    task_ctrl.execute_complete_task(CompleteTaskCommand(worker_id=worker1_id, task_id=task2.id, raw_output="recovered!"))
    ...
    run3_id = run_ctrl.execute_create_run(CreateRunCommand(
        session_id=uuid.uuid4(), benchmark_version_id=uuid.uuid4(), adapter_version_id=uuid.uuid4(), target_model="demo-model"
    ))
    run_ctrl.execute_validate_run(ValidateRunCommand(run_id=run3_id))
    scheduler.tick()
    print(f"[*] Run-3 started. Worker-1 assigned.")
    
    print(f"[*] User requests Cancellation...")
    ...
    print(f"[*] Worker-1 fails the task since it was aborted...")
    task3 = db_session.query(AtlasTask).filter_by(atlas_run_id=run3_id).one()
    task_ctrl.execute_fail_task(FailTaskCommand(worker_id=worker1_id, task_id=task3.id, error_code="ABORTED", error_message="Run cancelled", retryable=False))
    ...
    run4_id = run_ctrl.execute_create_run(CreateRunCommand(
        session_id=uuid.uuid4(), benchmark_version_id=uuid.uuid4(), adapter_version_id=uuid.uuid4(), target_model="demo-model"
    ))
    run_ctrl.execute_validate_run(ValidateRunCommand(run_id=run4_id))
    scheduler.tick()
    
    print(f"[*] Task fails persistently...")
    task4 = db_session.query(AtlasTask).filter_by(atlas_run_id=run4_id).one()
    
    # Attempt 1
    task_ctrl.execute_fail_task(FailTaskCommand(worker_id=worker1_id, task_id=task4.id, error_code="FATAL", error_message="Crash", retryable=True))
    task_ctrl.execute_recovery_decision(task4.id)
    scheduler.tick()
    
    # Attempt 2
    task_ctrl.execute_fail_task(FailTaskCommand(worker_id=worker1_id, task_id=task4.id, error_code="FATAL", error_message="Crash again", retryable=True))
    task_ctrl.execute_recovery_decision(task4.id)
