...
import pytest
from atlas_db.models.authoring import Benchmark, BenchmarkState
...
def test_concurrent_version_creation():
    ...
    version1 = service.create_version(
        benchmark_id=b1.id, version_string="v1.0", user_id=b1.author_id, user_role="project_write"
    )
    ...
    b2 = Benchmark(
        id=b1.id, status=BenchmarkState.DRAFT, project_id=b1.project_id, author_id=b1.author_id
    )
