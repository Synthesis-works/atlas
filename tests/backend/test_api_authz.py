...
def create_mock_jwt(user_id: str, org_id: str = None, mem_id: str = None):
    ...
    to_encode = {"sub": user_id, "exp": expire, "iat": iat, "jti": str(uuid4())}
    if org_id:
...
def mock_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.backend.authz.OrganizationMemberRepository", lambda db: mock_repo_instance
    )

    # Mock project repo inside authorize_project_access
    mock_project_repo = MagicMock()
    monkeypatch.setattr(
        "atlas_db.repositories.core.ProjectRepository", lambda db: mock_project_repo
    )
...
def test_cross_org_access(mock_dependencies):
    ...
    assert (
        response.json()["error"]["message"] == "You are not an active member of this organization."
    )
