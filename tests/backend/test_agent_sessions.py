...
def test_omitted_session_model_dispatches_provider_default_not_gemini(
    ...
    generic 'gemini-3.1-flash-lite' literal and Groq returned 404 model_not_found.
    ...
    if provider in ("groq", "mistral"):
        assert "gemini-3.1-flash-lite" not in json.dumps(payload)
...
def test_omitted_model_explicitly_groq_resolves_to_gpt_oss(db_session, monkeypatch):
    ...
    model_override='gemini-3.1-flash-lite'; the resolved model must be groq's
