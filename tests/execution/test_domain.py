...
from packages.execution_engine.domain.models import (
    ExecutionState, AttemptStatus, ExecutionAttempt, Lease
)
...
from packages.execution_engine.domain.events import (
    ExecutionQueuedEvent, ExecutionStartedEvent, ExecutionCancelledEvent, 
    ExecutionFailedEvent, ExecutionCompletedEvent, ExecutionRetryEvent
)

@pytest.fixture
def test_clock():
    return TestClock(datetime(2026, 1, 1, 12, 0, 0, tzinfo=timezone.utc))
...
def test_expire_lease_exhausts_retries(service, test_clock):
    ...
    from packages.execution_engine.domain.events import LeaseExpiredEvent, ExecutionRetryEvent
