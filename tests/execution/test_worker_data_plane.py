...
from packages.execution_engine.domain.exceptions import LeaseOwnershipError
...
def test_acquire_empty_queue(mock_worker_service):
    ...
    response = client.post(
        "/api/v1/internal/workers/acquire",
        json={"worker_id": str(uuid.uuid4()), "capabilities": []},
    )
...
def test_acquire_exactly_one(mock_worker_service):
    ...
    response = client.post(
        "/api/v1/internal/workers/acquire", json={"worker_id": str(worker_id), "capabilities": []}
    )
...
def test_lease_renewal(mock_worker_service):
    ...
    response = client.post(
        f"/api/v1/internal/workers/executions/{exec_id}/heartbeat",
...
def test_completion_after_expiry(mock_worker_service):
    ...
    response = client.post(
        f"/api/v1/internal/workers/executions/{exec_id}/complete_success",
        json={"worker_id": str(worker_id), "artifacts": []},
    )
    assert response.status_code == 403
...
def test_wrong_worker_heartbeat(mock_worker_service):
    ...
    exec_id = uuid.uuid4()
    worker_id = uuid.uuid4()
    mock_worker_service.heartbeat.side_effect = LeaseOwnershipError(
        "Worker does not own the active lease"
    )
