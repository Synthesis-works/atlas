...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "packages", "benchmark")))
...
def run_audit():
    with Session(engine) as session:
        ...
        dv = DatasetVersion(dataset_id=test_dataset.id, version_string="v1", storage_path="/", lifecycle="VALID")
        session.add(dv)
        session.flush()
        ...
        dp_he = DatasetPack(manifest=DatasetManifest(id="he", name="HE", version="1", source="s", license="l", citation="c", language="python", evaluation="execution", metric="pass", tasks=1, tags=[]), tasks=[he_task])
        ...
        check_rule = [r for r in rules_he if r.is_challenge is False and "def check(" in r.rule_definition]
        if check_rule:
            print("HE Test Extraction:", "SUCCESS" if check_rule[0].rule_definition == he_fixture["test"] + f"\ncheck({he_fixture['entry_point']})\n" else "FAILED")
        ...
        dp_mb = DatasetPack(manifest=DatasetManifest(id="mb", name="MB", version="1", source="s", license="l", citation="c", language="python", evaluation="execution", metric="pass", tasks=1, tags=[]), tasks=[mb_task])
        ...
        normal_rule = [r for r in rules_mb if r.is_challenge is False and "assert add" in r.rule_definition]
