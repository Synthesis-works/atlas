...
def attach_evaluation_strategy(session):
    print("[2.5/5] Attaching EXACT_MATCH evaluation strategy to seeded benchmarks...")
    from atlas_db.models.evaluation import EvaluationStrategy, EvaluationStrategyVersion

    strategy = session.query(EvaluationStrategy).first()
    if not strategy:
        strategy = EvaluationStrategy(name="Exact Match", type="exact_match")
        session.add(strategy)
        session.commit()

    version = (
        session.query(EvaluationStrategyVersion)
        .filter(EvaluationStrategyVersion.strategy_id == strategy.id)
        .first()
    )
    if not version:
        version = EvaluationStrategyVersion(strategy_id=strategy.id, version_string="v1.0")
        session.add(version)
        session.commit()

    attached = 0
    for bv in session.query(BenchmarkVersion).all():
        if not bv.evaluation_strategy_id:
            bv.evaluation_strategy_id = version.id
            attached += 1
    session.commit()
    print(f"    Attached strategy to {attached} benchmark version(s).")


def seed_humaneval_dataset(session, user, project):
    print("[2.6/5] Seeding HumanEval dataset + test cases (enables real API executions)...")
    ...
    humaneval = session.query(Benchmark).filter_by(name="HumanEval").first()
    humaneval_v1 = (
        session.query(BenchmarkVersion).filter_by(benchmark_id=humaneval.id).first()
        if humaneval
        else None
    )

    ds = session.query(Dataset).filter_by(name="HumanEval Suite").first()
    if not ds:
        ds = Dataset(
            project_id=project.id,
            name="HumanEval Suite",
            description="Sample HumanEval dataset enabling real execution dispatch",
            status=DatasetStatus.ACTIVE,
        )
        session.add(ds)
        session.commit()

    dv = session.query(DatasetVersion).filter_by(dataset_id=ds.id).first()
    if not dv:
        dv = DatasetVersion(
            dataset_id=ds.id,
            version_string="1.0.0",
            storage_path="demo/humaneval-v1",
    ...
    if humaneval_v1 and not humaneval_v1.primary_dataset_version_id:
        humaneval_v1.primary_dataset_version_id = dv.id
        session.commit()
    ...
    if session.query(TestCase).filter(TestCase.dataset_version_id == dv.id).count() == 0:
        ...
        for idx, (name, prompt_text, expected) in enumerate(samples):
            task = Task(
                ...
                benchmark_version_id=humaneval_v1.id if humaneval_v1 else None,
                name=f"HumanEval {name}",
                description="Sample task generated by seed_demo.",
                order_index=idx,
                metadata_={},
            )
            session.add(task)
            session.flush()
            session.add(Prompt(task_id=task.id, template="{prompt}", system_instruction=None))
            session.add(
                TestCase(
                    task_id=task.id,
                    dataset_version_id=dv.id,
...
def main():
    ...
    try:
        ...
        attach_evaluation_strategy(session)
        seed_humaneval_dataset(session, user, project)
