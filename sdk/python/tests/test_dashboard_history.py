        ...
        "activity": [
            {
                ...
                "title": "gemini-3.6-flash started on HellaSwag",
...
class TestDashboardSnapshotDto:
    ...
    def test_models_parse_live_payload_shape(self) -> None:
        ...
        assert activity.title == "gemini-3.6-flash started on HellaSwag"
...
class TestGetRecentExecutions:
    def test_success_parses_wrapped_list(self, httpx_mock: pytest.MockTransport) -> None:
        ...
        assert len(result) == 1
        execution = result[0]
        assert execution.target_model == "gemini-3.6-flash"
...
class TestGetRecentModels:
    def test_success_parses_wrapped_list(self, httpx_mock: pytest.MockTransport) -> None:
        ...
        assert len(result) == 2
        assert result[0].name == "gemini-3.6-flash"
