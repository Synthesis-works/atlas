...
def run_demo():
    ...
    run_id = run_ctrl.execute_create_run(
        CreateRunCommand(
    ...
    worker_a_id = worker_ctrl.execute_register_worker(
        RegisterWorkerCommand(
    ...
    print("[*] Worker-A claimed task. Simulating Worker-A dying...")
    ...
    worker_a.last_heartbeat_at = datetime.now(UTC) - timedelta(seconds=10)
    db_session.commit()
    ...
    worker_b_id = worker_ctrl.execute_register_worker(
        RegisterWorkerCommand(
            adapter_id=uuid.uuid4(),
            name="Worker-B",
            version="1.0",
    ...
    task_ctrl.execute_complete_task(
        CompleteTaskCommand(worker_id=worker_b_id, task_id=claimed_b[0], raw_output="recovered!")
    )
    ...
    worker_c_id = worker_ctrl.execute_register_worker(
        RegisterWorkerCommand(
            adapter_id=uuid.uuid4(),
            name="Worker-C",
            version="1.0",
    ...
    worker_c.last_heartbeat_at = datetime.now(UTC) - timedelta(
        seconds=3
    )  # > 2s unhealthy threshold, < 5s offline threshold
    db_session.commit()
    ...
    # Heartbeat returns
    worker_ctrl.execute_heartbeat(
        HeartbeatWorkerCommand(worker_id=worker_c_id, current_load=0, health="healthy")
    )
    worker_c = db_session.query(ExecutionWorker).filter_by(id=worker_c_id).one()
