...
3. **Every API task creation failed pydantic validation (`string_type`).**
   `TaskCreateRequest.model` is `Optional[str] = None`, but `AgentTask.model` is a required `str`. Passing `payload.model=None` explicitly raised a validation error. Fix in `apps/backend/routers/agent.py`: only assign `model` when provided, letting `AgentTask` keep its default (`gemini-3.1-flash-lite`).
...
|---|---|---|
| **Gemini** | Primary, default | `gemini-3.1-flash-lite`. Native `functionDeclarations`, schema sent unchanged. Quota is precious — do NOT run real-API tests casually. |
