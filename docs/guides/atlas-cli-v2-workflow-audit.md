...
| Submit a run | `atlas run submit <version-id> --target-model mock` | exit 0, QUEUED → RUNNING. (Note: default target is `gemini-3.6-flash` — a paid provider — beware cost.) |
...
**P4 — `run submit` has a costly default (high).** Default `--target-model gemini-3.6-flash`
