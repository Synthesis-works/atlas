...
| 4 | Safe submit defaults | CLI default target `gemini-1.5-flash` (paid provider) | CLI sets its own default that overrides the backend's cheaper default; no preflight | **CLI** (contract change) — no silent paid default; explicit target or clearly-safe default; add `--preview` |
