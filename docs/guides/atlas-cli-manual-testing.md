...
- **Options:** `--target-model TEXT` (**required** — no default; v2 removed the silent `gemini-1.5-flash` default), `--dataset-version-id TEXT` (default: resolved from benchmark version), `--preview` (read-only dry-run; **no POST**)
...
Report Runs
  ...
  --------  ----------------  -------  ------  -----  ----------------
  ff0a7a03  gemini-1.5-flash  1.0.0    FAILED  -      2026-08-26 06:24
```
...
- `run submit` with a real LLM target that yields actual API costs — the default `--target-model gemini-1.5-flash` hit a real provider adapter on this box; treat submit as potentially expensive.
