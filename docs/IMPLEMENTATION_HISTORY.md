...
- **Reason**: `run submit` defaulted to `gemini-3.1-flash-lite` — a paid real-provider model an agent could submit "innocently". v2 requires an explicit model and offers a no-POST plan.
...
### Slice 7: authoritative `atlas model list` (execution-target catalog)
- **Date**: August 2026
- **Purpose**: let agents and users ask "which models can I actually submit?" — the previous `GET /api/v1/models` was a dead stub calling a nonexistent `AdapterFactory.get_available_models()`, and nothing else enumerated execution targets.
- **Files changed**: new `apps/backend/adapters/registry.py` (`list_models()` enumerator + `DEFAULT_TARGET_MODELS` + `_provider_config_models`), new `apps/backend/schemas/models.py` (`ModelStatus`/`ModelRead`), `apps/backend/adapters/factory.py` (`get_available_models()` delegating to the registry), `apps/backend/routers/models.py` (auth-gated, `APIResponse[list[ModelRead]]`), `apps/backend/main.py` (mount `/api/v1/models`); SDK `atlas_sdk/models/models.py` (DTOs) + `AtlasClient.list_models()` + package exports + contract test; CLI `cli/cli/commands/models.py` (`model list`) + registration + tests; backend `tests/backend/test_models_catalog.py`; docs (plan status/commit order/done-criteria, manual-testing tree + `model list` section + e2e smoke, this file).
- **Reason**: no authoritative model catalog existed; three divergent candidate registries (client static lists, `config/providers.json`, agent `ModelRegistry`). Design was user-gated: enumerate on the execution-adapter side (shared `ProviderAdapter` client table + `config/providers.json` overrides + app defaults), auth-only (no org filtering), status mirrors the runtime `RealModelAdapter.predict` gate (`client.health()`). `NOT_CONFIGURED` ≠ invalid — the id still works for `--target-model`.
- **Impact**: `atlas model list` renders table/`--output json` (bare array)/`--quiet`/`--json-schema` (offline `array[ModelRead]`). Canonical `id` = `mock` or `provider/model`; client entries already carrying the `provider/` prefix are normalized (kills invalid `groq/groq/x`). Live-verified off the restarted backend: 16 canonical ids, `ollama/qwen2.5-coder:1.5b` correctly `NOT_CONFIGURED` (no live host), cloud providers `AVAILABLE` per key presence. A double-prefix regression (`groq/groq/…`, `nvidia/nvidia/…`) was caught live and fixed with a normalization regression test.
- **Current status**: Complete. Commit `a70d23d`. Backend catalog suite 11 passed; CLI suite 408 passed; SDK 177 passed/2 skipped; ruff + mypy clean on touched files.
