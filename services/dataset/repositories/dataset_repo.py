...
class DatasetRepository:
    ...
    def update_version_status(self, version_id: UUID, status: ValidationStatus) -> None:
        stmt = update(DatasetVersion).where(DatasetVersion.id == version_id).values(validation_status=status)
        self.db.execute(stmt)
        self.db.commit()

    def update_version_checksum(self, version_id: UUID, checksum: str) -> None:
        stmt = update(DatasetVersion).where(DatasetVersion.id == version_id).values(checksum=checksum)
        self.db.execute(stmt)
        self.db.commit()
