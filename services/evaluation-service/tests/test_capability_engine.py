...
def test_capability_engine():
    ...
    metrics = [
        MetricValueModel(
            name="pass_rate", value=0.9, normalized_value=0.9, category="CORRECTNESS", direction="HIGHER", unit="percent", source="exec", aggregation="mean"
        ),
        MetricValueModel(
            name="judge_score", value=0.8, normalized_value=0.8, category="QUALITY", direction="HIGHER", unit="score", source="judge", aggregation="mean"
        )
    ]
