...
from atlas_db.models.core import Project, User
...
def test_job_lifecycle(db_session):
    ...
    # 2. Start Attempt
    cmd_start = StartEvaluationAttemptCommand(job_id=job_id, pipeline_version_id=pipeline_version_id)
    ...
    # 3. Complete Attempt
    cmd_complete = CompleteEvaluationAttemptCommand(attempt_id=attempt_id, result_bundle=EvaluationResultBundle())
...
def test_cancel_job(db_session):
    ...
    with pytest.raises(ValueError, match="Cannot start attempt for job"):
        cmd_start2 = StartEvaluationAttemptCommand(job_id=job_id, pipeline_version_id=pipeline_version_id)
...
def test_concurrent_attempts(db_session):
    ...
    # Start First Attempt
    cmd_start1 = StartEvaluationAttemptCommand(job_id=job_id, pipeline_version_id=pipeline_version_id)
