...
class EvaluationOrchestrator:
    ...
    def handle_run_completed(self, run_id: UUID) -> None:
        ...
        execution_outputs = [
            {"text": out.raw_output, "tokens_used": out.tokens_used, "duration_ms": out.duration_ms}
            for out in outputs
        ]
        ...
        if not pipeline:
            pipeline = EvaluationPipeline(
                name="ExecutionPipeline", description="Default Execution Pipeline"
            )
        ...
        if not pipeline_version:
            pipeline_version = EvaluationPipelineVersion(
                pipeline_id=pipeline.id, version="1.0", config_schema={"k": 1}
            )
        ...
        cmd_start = StartEvaluationAttemptCommand(
            job_id=job_id, pipeline_version_id=pipeline_version.id
        )
        ...
        try:
            ...
            cmd_complete = CompleteEvaluationAttemptCommand(
                attempt_id=attempt_id, result_bundle=result_bundle
            )
            ...
            profile_model = self.capability_engine.process(
                run.adapter_version_id, result_bundle.metrics
            )

            # Persist Capability Profile
            profile = CapabilityProfile(adapter_version_id=profile_model.adapter_version_id)
            self.db.add(profile)
            self.db.flush()
