...
from atlas_db.models.evaluation import (
    EvaluationJob, EvaluationAttempt, EvaluationResult, 
    MetricValue, CapabilityProfile, CapabilityScore, MetricDefinition
)
from atlas_db.models.execution import ExecutionAdapterVersion, ExecutionAdapter
    ...
    for attempt in attempts:
        ...
        for res in results:
            ...
            attempt_data["results"].append({
                "result_id": res.id,
                "metrics": [{"name": m.metric_name, "value": m.value, "normalized": m.normalized_value} for m in metrics]
            })
...
def get_leaderboard(db: Session = Depends(get_db)):
    ...
    for profile in profiles:
        ...
        # Get adapter name
        adapter_version = db.query(ExecutionAdapterVersion).filter_by(id=profile.adapter_version_id).first()
...
def get_metrics(db: Session = Depends(get_db)):
    ...
    return [{"name": d.name, "category": d.category.value, "direction": d.direction.value, "unit": d.unit} for d in definitions]
