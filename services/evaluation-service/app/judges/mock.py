...
class MockJudgeProvider(JudgeProvider):
    ...
    def __init__(self, default_score: float = 1.0, default_reasoning: str = "Mock passed.", raise_on_prompt: Optional[str] = None):
    ...
    def evaluate(self, prompt: str, rubric: str) -> JudgeResponse:
        if self.raise_on_prompt and self.raise_on_prompt in prompt:
            raise ValueError(f"Mock failure triggered for prompt containing: {self.raise_on_prompt}")
