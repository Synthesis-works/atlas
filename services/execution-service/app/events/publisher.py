...
class EventPublisher(Protocol):
    def publish_event(self, run_id: str, event_type: EventType, message: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> None:
...
class PostgresEventPublisher:
    ...
    def publish_event(self, run_id: str, event_type: EventType, message: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        event = RunEvent(
            atlas_run_id=uuid.UUID(str(run_id)),
            type=event_type,
            message=message,
            metadata_=metadata
        )
