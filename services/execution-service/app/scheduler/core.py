...
from atlas_db.models.execution import (
    AtlasTask, ExecutionWorker, AtlasRun, TaskStatus, WorkerStatus, EventType
)
from app.commands.task import ClaimTasksCommand
from app.controllers.task_controller import TaskController
from app.events.publisher import EventPublisher
from app.scheduler.policies import CompositePolicy
from app.scheduler.models import SchedulingDecision, SchedulerMetrics

logger = logging.getLogger(__name__)

class AtlasScheduler:
    def __init__(self, db: Session, task_controller: TaskController, event_publisher: EventPublisher, policy: CompositePolicy):
        self.db = db
    ...
    def tick(self):
        ...
        tasks = self.db.query(AtlasTask).join(AtlasRun).filter(
            and_(
                AtlasTask.status == TaskStatus.PENDING,
                AtlasRun.status.in_([RunStatus.QUEUED, RunStatus.RUNNING])
            )
        ).order_by(
            AtlasTask.priority.desc(),
            AtlasTask.created_at.asc()
        ).all()
        ...
        global_running = self.db.query(func.count(AtlasTask.id)).filter(AtlasTask.status == TaskStatus.RUNNING).scalar()
        
        for task in tasks:
            self.metrics.record_examined()
            
            # Fetch model-specific running counts
            model_running = self.db.query(func.count(AtlasTask.id)).join(AtlasRun).filter(
                and_(AtlasTask.status == TaskStatus.RUNNING, AtlasRun.target_model == task.run.target_model)
            ).scalar()
            
            # Find an available worker (Ordered by load ASC for simple load balancing)
            workers = self.db.query(ExecutionWorker).filter(
                ExecutionWorker.status == WorkerStatus.READY
            ).order_by(
                ExecutionWorker.current_load.asc()
            ).all()
            ...
            worker_running = self.db.query(func.count(AtlasTask.id)).filter(
                and_(AtlasTask.status == TaskStatus.RUNNING, AtlasTask.assigned_worker_id == worker.id)
            ).scalar()
