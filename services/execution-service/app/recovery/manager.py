...
class RecoveryManager:
    ...
    def _detect_expired_leases(self, now: datetime):
        ...
        tasks = self.db.query(AtlasTask).filter(
    ...
    def _detect_offline_workers(self, now: datetime):
        ...
        workers = self.db.query(ExecutionWorker).filter(
            ExecutionWorker.status.in_([WorkerStatus.READY, WorkerStatus.BUSY, WorkerStatus.UNHEALTHY]),
            ExecutionWorker.last_heartbeat_at != None,
            ExecutionWorker.last_heartbeat_at < offline_time
        ).all()
    ...
    def _emit_worker_event(self, worker: ExecutionWorker, msg: str):
        self.event_publisher.publish_event(
            ...
            message=msg,
            metadata={"worker_id": str(worker.id), "status": "UNHEALTHY" if "UNHEALTHY" in msg else "OFFLINE"}
        )
