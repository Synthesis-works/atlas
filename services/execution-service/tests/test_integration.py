...
from app.commands.run import CreateRunCommand, ValidateRunCommand, CancelRunCommand
from app.commands.worker import RegisterWorkerCommand, HeartbeatWorkerCommand
from app.commands.task import ClaimTasksCommand, CompleteTaskCommand, FailTaskCommand
...
def test_happy_path(system):
    # Test 1: Happy Path
    run_id = system["run_ctrl"].execute_create_run(CreateRunCommand(
        session_id=uuid.uuid4(), benchmark_version_id=uuid.uuid4(), adapter_version_id=uuid.uuid4(), target_model="demo"
    ))
    system["run_ctrl"].execute_validate_run(ValidateRunCommand(run_id=run_id))
    
    worker_id = system["worker_ctrl"].execute_register_worker(RegisterWorkerCommand(
        adapter_id=uuid.uuid4(), name="w1", version="1.0", hostname="h1", platform="linux", region="us", hardware_info={}, capabilities={}
    ))
    ...
    system["task_ctrl"].execute_complete_task(CompleteTaskCommand(worker_id=worker_id, task_id=task.id, raw_output="ok"))
...
def test_worker_failure_and_recovery(system):
    ...
    worker1_id = system["worker_ctrl"].execute_register_worker(RegisterWorkerCommand(
        adapter_id=uuid.uuid4(), name="w1", version="1.0", hostname="h1", platform="linux", region="us", hardware_info={}, capabilities={}
    ))
    worker2_id = system["worker_ctrl"].execute_register_worker(RegisterWorkerCommand(
        adapter_id=uuid.uuid4(), name="w2", version="1.0", hostname="h2", platform="linux", region="us", hardware_info={}, capabilities={}
    ))
    ...
    system["task_ctrl"].execute_complete_task(CompleteTaskCommand(worker_id=assigned_worker, task_id=task.id, raw_output="ok"))
...
def test_user_cancellation(system):
    ...
    task = system["db"].query(AtlasTask).filter_by(atlas_run_id=run_id).one()
    system["task_ctrl"].execute_fail_task(FailTaskCommand(worker_id=worker_id, task_id=task.id, error_code="ABORTED", error_message="cancelled", retryable=False))
...
def test_retry_exhaustion(system):
    ...
    # Fail 1
    system["task_ctrl"].execute_fail_task(FailTaskCommand(worker_id=worker_id, task_id=task.id, error_code="FATAL", error_message="err", retryable=True))
    system["task_ctrl"].execute_recovery_decision(task.id)
    ...
    system["task_ctrl"].execute_fail_task(FailTaskCommand(worker_id=assigned, task_id=task.id, error_code="FATAL", error_message="err", retryable=True))
    system["task_ctrl"].execute_recovery_decision(task.id)
...
def test_recovery_skipped(system):
    ...
    worker.last_heartbeat_at = datetime.now(timezone.utc) - timedelta(seconds=3)
    system["db"].commit()
    ...
    # Heartbeat returns
    system["worker_ctrl"].execute_heartbeat(HeartbeatWorkerCommand(worker_id=worker_id, current_load=0, health="healthy"))
    worker = system["db"].query(ExecutionWorker).filter_by(id=worker_id).one()
