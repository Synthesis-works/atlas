...
from app.scheduler.policies import (
    GlobalConcurrencyPolicy, PerWorkerConcurrencyPolicy, 
    PerModelConcurrencyPolicy, CompositePolicy
)
...
def setup_mock_state(db_session):
    ...
    run = AtlasRun(
        ...
        adapter_version_id=uuid.uuid4(), target_model="gpt-4", status=RunStatus.QUEUED, total_tasks=3
    )
    db_session.add(run)
    
    # Setup Tasks (Mixed priorities)
    t1 = AtlasTask(id=uuid.uuid4(), atlas_run_id=run_id, test_case_id=uuid.uuid4(), status=TaskStatus.PENDING, priority=0)
    t2 = AtlasTask(id=uuid.uuid4(), atlas_run_id=run_id, test_case_id=uuid.uuid4(), status=TaskStatus.PENDING, priority=10) # High priority
    t3 = AtlasTask(id=uuid.uuid4(), atlas_run_id=run_id, test_case_id=uuid.uuid4(), status=TaskStatus.PENDING, priority=0)
