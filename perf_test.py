...
async def run_perf():
    engine = create_async_engine('postgresql+asyncpg://postgres:postgres@localhost:5432/test_alembic_isolation', pool_pre_ping=True)
    ...
    async with async_session() as session:
        session.add(ds.Dataset(id=ds_id, project_id=uuid.uuid4(), name='perf', version='1', license_id=uuid.uuid4(), registry_id=uuid.uuid4(), source_id=uuid.uuid4()))
        session.add(ds.DatasetVersion(id=dsv_id, dataset_id=ds_id, version_hash='x'))
        await session.commit()
        
        async def make_tasks(n):
            for i in range(n):
                t = ts.Task(id=uuid.uuid4(), title=f't{i}', dataset_version_id=dsv_id, input_data={}, expected_output={}, evaluation_strategy='x')
                session.add(t)
                session.add(ts.Prompt(id=uuid.uuid4(), task_id=t.id, system_prompt='s', user_prompt='u'))
                session.add(ts.TestCase(id=uuid.uuid4(), task_id=t.id, input_data={'a':1}, expected_output={'b':1}, is_canonical=True))
            await session.commit()
