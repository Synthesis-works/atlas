...
org_projects_router = APIRouter(
    prefix="/organizations/{org_id}/projects", tags=["Organization Projects"]
)
...
@org_projects_router.post(
    "", response_model=APIResponse[ProjectRead], status_code=status.HTTP_201_CREATED
)
def create_project(
    org_id: UUID,
    data: ProjectCreate,
    member: OrganizationMember = Depends(
        require_role([OrganizationRole.MEMBER, OrganizationRole.ADMIN, OrganizationRole.OWNER])
    ),
    project_service: ProjectService = Depends(get_project_service),
):
    project = project_service.create(org_id, member_id=member.id, data=data)
    return APIResponse.success_response(data=project, message="Project created successfully")
