...
class ConcurrencyPolicy(ABC):
    ...
    def can_schedule(
        self,
        task,
        current_running_global: int,
        current_running_worker: int,
        current_running_model: int,
    ) -> bool:
        """
...
class CompositePolicy(ConcurrencyPolicy):
    ...
    def can_schedule(
        ...
        for policy in self.policies:
            if not policy.can_schedule(
                task, current_running_global, current_running_worker, current_running_model
            ):
                return False
        return True
