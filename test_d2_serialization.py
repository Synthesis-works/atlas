...
def run_serialization_matrix(engine):
    with Session(engine) as session:
        ...
        def execute_case(expected_output, target_assertion):
            ...
            tc = TestCase(task_id=t.id, input_data={}, expected_output=expected_output, is_hidden=False)
        ...
        tests = [
            ...
            ("5. Unwrapped list", ["assert a == 1", "assert b == 2"], '["assert a == 1", "assert b == 2"]'),
            ("6. Numeric value", 5, "5"),
            ("7. Boolean value", True, "true"),
            ("8. Null value", None, "null"),
            ("9. Nested mixed JSON", {"a": [1, {"b": True}], "c": None}, '{"a": [1, {"b": true}], "c": null}'),
            ("10. HumanEval Canonical", {"output": "    return a + b"}, "    return a + b"),
            ("11. MBPP Canonical", {"output": "def sum(a, b): return a + b"}, "def sum(a, b): return a + b"),
        ]
