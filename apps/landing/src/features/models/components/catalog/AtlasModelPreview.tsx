...
export function AtlasModelPreview({ catalog }: { catalog: ReturnType<typeof useModelCatalog> }) {
  const { previewModel, handleClosePreview } = catalog;
  const activeTab = useWorkspaceInteractionStore(s => s.workspaces['models']?.view.activePreviewTab || 'Overview');
  ...
  const savedScroll = useWorkspaceInteractionStore(s => s.workspaces['models']?.navigation.scrollPosition || 0);
  const scrollRef = useRef<HTMLDivElement>(null);

  const tabs = [
    'Overview', 'Benchmarks', 'Versions', 'Security', 'Cost'
  ];
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        onScroll={(e) => {
          setScrollPosition('models', (e.target as HTMLDivElement).scrollTop);
        }}
      >
        {/* Header */}
          ...
          <div className="flex gap-4 items-start pr-8">
            <div className="flex flex-col gap-1 w-full">
              <div className="flex flex-col">
                ...
                <p className="text-white/50 text-sm">Provider: {previewModel.provider}</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mt-4 bg-white/5 p-3 rounded-lg border border-white/10 w-full">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-white/50">Arch: <span className="text-white/80">{previewModel.architecture}</span></span>
                    <span className="text-xs text-white/50">Params: <span className="text-white/80">{previewModel.parameterCount}</span></span>
                  </div>
                </div>
                <div className="flex flex-col gap-2 border-l border-white/10 pl-4">
                  <h4 className="text-[10px] text-white/30 uppercase tracking-widest font-semibold">Performance</h4>
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-white/50">Score: <span className="text-emerald-400">{previewModel.overallScore}</span></span>
                    <span className="text-xs text-white/50">Latency: <span className="text-white/80">{previewModel.latencyMs}ms</span></span>
                    <span className="text-xs text-white/50">Context: <span className="text-white/80">{previewModel.contextWindowFormatted}</span></span>
                  </div>
                </div>
                <div className="flex flex-col gap-2 border-l border-white/10 pl-4">
                  <h4 className="text-[10px] text-white/30 uppercase tracking-widest font-semibold">Cost & Health</h4>
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-white/50">Cost: <span className="text-white/80">{previewModel.costEstimate}</span></span>
                    <span className="text-xs text-white/50">Status: <span className="text-white/80">{previewModel.healthLabel}</span></span>
                    <span className="text-xs text-white/50">Avail: <span className="text-white/80">{previewModel.availability}%</span></span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="sticky top-0 z-10 bg-black/80 backdrop-blur-md flex items-center overflow-x-auto border-b border-white/10 px-6 no-scrollbar hide-scrollbar">
          {tabs.map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab('models', tab)}
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {previewModel.capabilityTags.map(tag => (
