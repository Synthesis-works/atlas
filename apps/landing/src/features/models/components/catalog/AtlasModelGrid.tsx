...
export function AtlasModelGrid({ catalog }: { catalog: ReturnType<typeof useModelCatalog> }) {
                ...
                <div className="p-6 relative flex-1 overflow-y-auto">
                  ...
                  >
                    <div className="grid grid-cols-3 gap-4 p-4 rounded-xl bg-white/5 border border-white/5">
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Context</span>
                        <span className="text-white">{activeCard.contextWindowFormatted}</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Latency</span>
                        <span className="text-white">{activeCard.latencyMs}ms</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Health</span>
                        <span className="text-white">{activeCard.healthLabel}</span>
                      </div>
                    </div>
                  </motion.div>
                </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col p-5 cursor-pointer mt-4">
                ...
                >
                  <span>{card.provider} • Score: {card.overallScore}</span>
                  <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${card.status === 'active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                    {card.status}
                  </span>
                </motion.p>
              </div>
