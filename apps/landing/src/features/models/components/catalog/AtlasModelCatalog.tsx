...
import { AtlasModelGrid } from './AtlasModelGrid';
import { AtlasModelTable } from './AtlasModelTable';
import { AtlasModelPreview } from './AtlasModelPreview';
import { AtlasModelEmptyState } from './AtlasModelEmptyState';
import { AtlasModelErrorState } from './AtlasModelErrorState';
import { AtlasModelSkeleton } from './AtlasModelSkeleton';
...
export function AtlasModelCatalog() {
    ...
    <AnimatedSection className="flex flex-col gap-6 w-full relative" delay={100}>
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between min-h-[64px] py-4 lg:py-2 border border-white/10 rounded-xl px-4 bg-black/40 gap-4 w-full">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full lg:w-auto">
          ...
          <button
            onClick={() => {
              useWorkspaceInteractionStore.getState().resetWorkspace('models');
              catalog.handleSearch('');
              catalog.handleClearSelection();
              catalog.setSort({ field: 'score', direction: 'desc' });
            }}
      ...
      <div className="flex flex-col xl:flex-row items-start gap-6 w-full relative">
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          {catalog.error ? (
            <AtlasModelErrorState catalog={catalog} />
          ) : catalog.isLoading ? (
            <AtlasModelSkeleton />
          ) : catalog.totalItems === 0 ? (
            <AtlasModelEmptyState />
          ) : catalog.viewMode === 'grid' ? (
            <AtlasModelGrid catalog={catalog} />
          ) : (
            <AtlasModelTable catalog={catalog} />
          )}
        </div>

        {/* Preview Panel */}
        <AnimatePresence>
          {catalog.previewId && <AtlasModelPreview catalog={catalog} />}
        </AnimatePresence>
      </div>
