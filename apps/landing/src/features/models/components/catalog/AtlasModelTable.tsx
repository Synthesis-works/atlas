...
export function AtlasModelTable({ catalog }: { catalog: ReturnType<typeof useModelCatalog> }) {
        ...
        <table className="w-full text-left text-sm text-white/70">
          <thead className="bg-black/40 border-b border-white/10 text-white/50 text-xs uppercase tracking-wider">
            <tr>
              ...
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('score')}>
                Score <SortIcon field="score" />
              </th>
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('latency')}>
                Latency <SortIcon field="latency" />
              </th>
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('context')}>
            ...
            {rows.map(row => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">{row.overallScore}</td>
                  <td className="p-4 whitespace-nowrap">{row.latencyMs}ms</td>
                  <td className="p-4 whitespace-nowrap">{row.contextWindowFormatted}</td>
                  <td className="p-4 whitespace-nowrap">{row.updatedAt}</td>
                  <td className="p-4">{row.provider}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${row.status === 'active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
