...
export function AtlasDatasetGrid({ catalog }: { catalog: ReturnType<typeof useDatasetCatalog> }) {
            ...
            >
              <motion.div layoutId={`image-${activeCard.id}-${id}`}>
                <img
                  ...
                  alt={activeCard.name}
                  className="w-full h-64 sm:rounded-tr-3xl sm:rounded-tl-3xl object-cover object-center"
                />
              </motion.div>
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="flex justify-between items-start p-6 border-b border-white/10">
                  <div className="flex flex-col gap-1">
                    ...
                    >
                      <span>{activeCard.provider}</span>
                      <span>•</span>
                      <span>{activeCard.version}</span>
                      <span>•</span>
                ...
                <div className="p-6 relative flex-1 overflow-y-auto">
                  ...
                  >
                    {/* Expanded Section Order: Overview, Description, Metadata, Storage, Quality, Relationships, Models, Benchmarks, Version History, Recent Activity, Tags, Actions */}
                    <div className="grid grid-cols-3 gap-4 p-4 rounded-xl bg-white/5 border border-white/5">
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Storage</span>
                        <span className="text-white">{activeCard.sizeFormatted}</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Samples</span>
                        <span className="text-white">{activeCard.samplesFormatted}</span>
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-xs uppercase tracking-wider text-white/40">Health</span>
                        <span className="text-white">{activeCard.healthLabel} ({activeCard.healthScore})</span>
                      </div>
                    </div>
                    
                    <div>
                      <h4 className="text-white mb-2">Description</h4>
                      <p>Expanded details for {activeCard.name} go here. This mimics the detailed view required by the spec without duplicating the full preview panel.</p>
                    </div>
                  </motion.div>
                </div>
              </div>
          ...
          return (
            ...
            >
              <div className="relative w-full">
                {/* Selection Checkbox (Stops propagation to avoid opening preview/expanding) */}
                ...
                <motion.div layoutId={`image-${card.id}-${id}`} className="cursor-pointer">
                  <img
                    width={400}
                    height={200}
                    src={card.thumbnailUrl}
                    alt={card.name}
                    className="h-48 w-full object-cover object-center pointer-events-none"
                  />
                </motion.div>
              </div>
              ...
              <div className="flex flex-col p-5 cursor-pointer">
                ...
                >
                  <span>{card.sizeFormatted} • {card.samplesFormatted}</span>
                  <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${card.status === 'READY' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                    {card.status}
                  </span>
                </motion.p>
              </div>
