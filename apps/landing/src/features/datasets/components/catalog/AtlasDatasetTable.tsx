...
export function AtlasDatasetTable({ catalog }: { catalog: ReturnType<typeof useDatasetCatalog> }) {
        ...
        <table className="w-full text-left text-sm text-white/70">
          <thead className="bg-black/40 border-b border-white/10 text-white/50 text-xs uppercase tracking-wider">
            <tr>
              ...
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('updated')}>
                Updated <SortIcon field="updated" />
              </th>
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('storage')}>
                Storage <SortIcon field="storage" />
              </th>
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('samples')}>
                Samples <SortIcon field="samples" />
              </th>
              <th className="p-4 cursor-pointer group hover:text-white transition-colors" onClick={() => handleSort('health')}>
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <img src={row.thumbnailUrl} alt={row.name} className="w-8 h-8 rounded object-cover" />
                      <div className="flex flex-col">
                        <span className="text-white font-medium">{row.name}</span>
                        <span className="text-xs text-white/40">{row.provider} • {row.version}</span>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 whitespace-nowrap">{row.updatedAt}</td>
                  <td className="p-4 whitespace-nowrap">{row.sizeFormatted}</td>
                  <td className="p-4 whitespace-nowrap">{row.samplesFormatted}</td>
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${row.healthScore >= 90 ? 'bg-emerald-400' : row.healthScore >= 70 ? 'bg-amber-400' : 'bg-red-400'}`}></span>
                      {row.healthScore}%
                    </div>
                  </td>
                  <td className="p-4">{row.owner}</td>
                  <td className="p-4">
                    <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${row.status === 'READY' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
