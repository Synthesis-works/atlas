...
class ExecutionService:
    ...
    def create_execution(
        ...
        benchmark_version = db.query(BenchmarkVersion).filter(
            BenchmarkVersion.id == execution_in.benchmark_version_id
        ).first()

        if not benchmark_version:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="BenchmarkVersion not found"
            )
    ...
    def update_status(
        ...
        if current == ExecutionStatus.QUEUED:
            allowed = {ExecutionStatus.RUNNING, ExecutionStatus.CANCELLED, ExecutionStatus.FAILED, ExecutionStatus.TIMED_OUT}
        ...
        # Allowed transitions from RUNNING
            ...
            allowed = {ExecutionStatus.COMPLETED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED, ExecutionStatus.TIMED_OUT}
            if new_status not in allowed:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
