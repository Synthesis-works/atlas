...
class DatasetService:
    ...
    def list_datasets(self, project_id: uuid.UUID, skip: int = 0, limit: int = 100) -> Sequence[Dataset]:
        stmt = select(Dataset).where(
            Dataset.project_id == project_id, 
            Dataset.status != DatasetStatus.ARCHIVED
        ).offset(skip).limit(limit)
        return self.dataset_repo.session.execute(stmt).scalars().all()

    def create_dataset(self, project_id: uuid.UUID, member_id: uuid.UUID, data: DatasetCreate) -> Dataset:
        dataset = Dataset(
