...
class MockProviderScenario(BaseLLMProvider):
    ...
    def decide(self, task: AgentTask, prompt_context: str, available_tools: list) -> AgentDecision:
        ...
        if dtype == AgentDecisionType.FAIL:
            ...
            args = dict(curr.get("arguments", {}))

            # Dynamically replace mock UUIDs with the real ones generated by earlier tools
            # to satisfy database foreign-key and existence validation in the tools.
            if task.benchmark_id and args.get("benchmark_id") == "mock-bm-id":
                args["benchmark_id"] = str(task.benchmark_id)
            if task.dataset_id and args.get("dataset_id") == "mock-ds-id":
                args["dataset_id"] = str(task.dataset_id)

            if (
                task.execution_ids
                and len(task.execution_ids) > 0
                and args.get("execution_id") == "mock-exec-id"
            ):
                args["execution_id"] = str(task.execution_ids[0])

            for trace in task.execution_trace:
                if (
                    getattr(trace, "event_type", getattr(trace, "action", None))
                    == "TOOL_EXECUTION_SUCCESS"
                ):
                    tool_name = trace.details.get("tool_name")
                    result = trace.details.get("output", {})
                    if (
                        tool_name == "create_benchmark"
                        and args.get("benchmark_version_id") == "mock-bmv-id"
                    ):
                        if result.get("version_id"):
                            args["benchmark_version_id"] = str(result["version_id"])
                    if (
                        tool_name == "create_dataset"
                        and args.get("dataset_version_id") == "mock-dsv-id"
                    ):
                        if result.get("version_id"):
                            args["dataset_version_id"] = str(result["version_id"])

            return AgentDecision(type=dtype, tool_name=curr.get("tool_name"), arguments=args)
        else:
...
def test_scenario_j_k_full_workflow_and_lineage_isolation():
    ...
    os.environ["AGENT_INLINE_EXECUTION_WAIT"] = "true"
    from apps.backend.config import settings

    settings.agent_inline_execution_wait = True
    import atlas_db.models  # noqa
    import packages.execution_engine.persistence.models  # noqa
    import apps.backend.worker.tasks

    apps.backend.worker.tasks.run_execution_task.delay = lambda *args, **kwargs: None
    engine = create_engine("sqlite:///:memory:")
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)
    db = Session()
    ...
    workflow_responses = [
        ...
        },
        {
            ...
            "arguments": {"dataset_id": "mock-ds-id"},
        },
        {
            "type": AgentDecisionType.TOOL_CALL,
            "tool_name": "run_benchmark",
            "arguments": {
                "benchmark_version_id": "mock-bmv-id",
        ...
        },
        {
            ...
            "arguments": {"execution_id": "mock-exec-id"},
        },
        {
            "type": AgentDecisionType.TOOL_CALL,
