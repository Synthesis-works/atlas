...
from packages.llm.clients.nvidia import NvidiaClient
...
class NvidiaAgentProvider(BaseLLMProvider):
    """
    Agent LLM provider using Nvidia.com inference API.
    ...
    def decide(
        ...
        system_instruction = (
            "You are the Atlas Agent powered by Nvidia/LLaMA, an autonomous execution engine for AI benchmarking.\n"
        ...
        if "tool_calls" in message and message["tool_calls"]:
            ...
            if tool_name:
                logger.info(f"Nvidia selected native tool '{tool_name}' with args: {arguments}")
                return AgentDecision(
                    type=AgentDecisionType.TOOL_CALL,
                    tool_name=tool_name,
                    arguments=arguments,
                    reasoning=f"Nvidia ({self.model}) selected native tool '{tool_name}'",
                )
            ...
            if parsed and "tool_name" in parsed:
                ...
                if tool_name:
                    return AgentDecision(
                        ...
                        tool_name=tool_name,
                        arguments=arguments,
                        reasoning=f"Nvidia ({self.model}) selected tool '{tool_name}' via JSON text",
                    )

        return AgentDecision(
            type=AgentDecisionType.FINAL_RESPONSE,
            response=content or "No response generated.",
            reasoning=f"Nvidia ({self.model}) produced text response",
        )
