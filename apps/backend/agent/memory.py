...
class SemanticMemoryItem:
    def __init__(self, memory_id: str, memory_type: str, text: str, embedding: list[float], metadata: dict[str, Any]):
...
class SemanticMemoryStore:
        ...
        try:
            with httpx.Client(timeout=2.0) as client:
                ...
                if res.status_code == 200:
                    ...
                    if target_model:
                        self.provider = OllamaEmbeddingProvider(model=target_model, base_url=self.base_url)
                        self.is_available = True
                        logger.info(f"SemanticMemoryStore initialized with Ollama model: '{target_model}'")
                    else:
    ...
    def add_memory(self, memory_id: str, memory_type: str, text: str, metadata: dict[str, Any]) -> bool:
...
class AgentMemoryManager:
    ...
    def build_prompt_context(self, task: AgentTask) -> str:
        ...
        if task.benchmark_id:
            lines.append(f"  - BENCHMARK CREATED: benchmark_id='{task.benchmark_id}', benchmark_version_id='{task.benchmark_version_id}'. (DO NOT call search_benchmarks or create_benchmark again).")
        else:
            lines.append("  - BENCHMARK: Not created yet.")

        if task.dataset_id:
            lines.append(f"  - DATASET CREATED: dataset_id='{task.dataset_id}'. (DO NOT call create_dataset again; call create_evaluation_case next).")
        else:
        ...
        if eval_cases:
            lines.append(f"  - EVALUATION CASES CREATED: {len(eval_cases)} evaluation cases defined (DO NOT call create_evaluation_case again; call validate_benchmark_dataset next).")
        else:
            lines.append("  - EVALUATION CASES: Not created yet.")

        if task.execution_ids:
            lines.append(f"  - EXECUTIONS COMPLETED: execution_ids={task.execution_ids}. (DO NOT call run_benchmark or get_run_status again; call evaluate_run).")

        if task.report_id:
            lines.append(f"  - REPORT PUBLISHED: report_id='{task.report_id}'. Task workflow complete.")
