...
class ExecutionWorker:
    ...
    def mark_timed_out(self, execution_id: uuid.UUID, correlation_id: str = None):
        ...
        if execution and execution.status in (ExecutionStatus.QUEUED, ExecutionStatus.RUNNING):
            ...
            self.db.commit()
            self.event_bus.emit(ExecutionFailed(
    ...
    def process(self, execution_id: uuid.UUID, correlation_id: str = None):
        ...
        if execution.status != ExecutionStatus.QUEUED:
            logger.warning(f"Execution {execution_id} is not QUEUED. Current status: {execution.status}")
            return

        execution.status = ExecutionStatus.RUNNING
        self.db.commit()
        
        self.event_bus.emit(ExecutionStarted(
            execution_id=execution_id,
            aggregate_id=execution_id,
            correlation_id=correlation_id,
            event_time=datetime.now(timezone.utc)
        ...
        try:
            ...
            if execution.cancellation_requested:
                ...
                self.db.commit()
                self.event_bus.emit(ExecutionCancelled(
                    execution_id=execution_id,
                    aggregate_id=execution_id,
                    correlation_id=correlation_id,
                    event_time=datetime.now(timezone.utc)
        ...
        self.event_bus.emit(ExecutionCompleted(
            execution_id=execution_id,
            aggregate_id=execution_id,
            correlation_id=correlation_id,
            event_time=datetime.now(timezone.utc)
