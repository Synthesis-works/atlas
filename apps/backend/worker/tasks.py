...
def run_execution_task(self, execution_id_str: str, correlation_id: str = None):
    ...
    try:
        ...
        logger.warning("Execution timed out (SoftTimeLimitExceeded)", execution_id=str(execution_id))
        with SessionLocal() as db:
            worker = ExecutionWorker(db)
        ...
        if self.request.retries >= self.max_retries:
            logger.error("Max retries exceeded, execution permanently failed", dead_letter=dead_letter_payload)
