...
class ExecutionRunner:
    ...
    def run(self, execution: Execution) -> List[ModelOutput]:
        ...
        tasks = self.db.query(Task).filter(Task.benchmark_version_id == execution.benchmark_version_id).all()
        ...
        for task in tasks:
            ...
            for test_case in task.test_cases:
                ...
                if batch_count >= batch_size or (current_time - last_commit_time) >= 5.0:
                    ...
                    logger.info(f"Execution {execution.id} progress: {execution.completed_items}/{execution.total_items}")
