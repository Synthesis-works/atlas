...
from packages.execution_engine.domain.events import ExecutionCompletedEvent, DomainEvent
...
class EvaluationSubscriber(EventSubscriber):
    ...
    def __init__(self):
        ...
        class ExactMatchEvaluator(BaseEvaluator):
            def prepare(self, context: EvaluatorContext) -> None: pass
            def evaluate(self, execution_output: dict) -> RawMeasurements:
                return RawMeasurements({"exact_match": True, "latency": 150})
            def postprocess(self, measurements: RawMeasurements) -> None: pass
        ...
        class ExactMatchScoring(BaseScoringStrategy):
            def score(self, measurements: RawMeasurements) -> CapabilityProfile:
                ...
                return CapabilityProfile(
                    ...
                    explanation={"overall": overall, "breakdown": {"Reasoning": overall}, "weights": {"Reasoning": 1.0}}
                )
    ...
    def handle(self, event: DomainEvent) -> None:
        if isinstance(event, ExecutionCompletedEvent):
            logger.info("EvaluationSubscriber received ExecutionCompletedEvent", execution_id=str(event.execution_id))
