...
from atlas_db.models.evaluation import EvaluationResult, CapabilityProfile, EvaluationStatus, EvaluationArtifact
from atlas_db.models.execution import Execution
from atlas_db.models.execution import ModelOutput # Ensure model_outputs is available
from packages.evaluation_engine.domain.events import EvaluationStartedEvent, EvaluationCompletedEvent, EvaluationFailedEvent
...
class EvaluationAppService:
    ...
    def evaluate_execution(self, execution_id: uuid.UUID) -> None:
        ...
        strategy_version_id = uuid.uuid4() # In a real implementation, fetched from execution.benchmark
        ...
        try:
            ...
            model_output_id = execution.model_outputs[0].id if execution.model_outputs else uuid.uuid4()
            ...
            logger.error("Evaluation pipeline failed", execution_id=str(execution_id), exc_info=True)
    ...
    def _publish_completed(self, evaluation_id: uuid.UUID, execution_id: uuid.UUID, overall_score: float, artifact_count: int):
