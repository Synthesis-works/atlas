...
async def test_d4_idempotent_export_action(pg_session):
    ...
    print("Statuses:", res)
    indexes = pg_session.execute(
        text("SELECT indexdef FROM pg_indexes WHERE indexname = 'idx_unique_active_dataset_export'")
    ).fetchall()
...
async def test_api_scheduling_endpoint_auth(pg_session):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        resp = await client.post(
            f"/api/v1/projects/{uuid.uuid4()}/datasets/{uuid.uuid4()}/exports", headers={}
        )
