...
class DatasetExportService:
    ...
    def __init__(
        ...
        self.exporters = exporters or {"jsonl": JSONLDatasetExporter()}
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                uri = self.artifact_store.store_training_artifact(
                    dataset_version_id=dataset_version_id, name=filename, source_path=tmp_path
                )

            return uri
        except Exception as e:
            logger.error(
                "Failed to export dataset", dataset_version_id=str(dataset_version_id), error=str(e)
            )
