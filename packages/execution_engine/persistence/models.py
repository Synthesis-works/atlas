...
class LeaseModel(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    attempt_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("execution_attempts.id", ondelete="CASCADE"), nullable=False, unique=True)
    worker_id: Mapped[uuid.UUID] = mapped_column(nullable=False, index=True)
    acquired_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, index=True)
    
    attempt: Mapped["ExecutionAttemptModel"] = relationship("ExecutionAttemptModel", back_populates="lease")

class ArtifactModel(Base):
    __tablename__ = "execution_artifacts"
    
    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    attempt_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("execution_attempts.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    attempt: Mapped["ExecutionAttemptModel"] = relationship("ExecutionAttemptModel", back_populates="artifacts")
