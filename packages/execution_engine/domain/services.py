...
class ExecutionService:
    ...
    def _assert_not_terminal(self, execution: Execution):
        if execution.is_terminal():
            raise ImmutableExecutionError(
                f"Execution {execution.id} is in terminal state {execution.status.name} and cannot be modified."
            )

    def create_execution(
        self, execution_id: uuid.UUID, benchmark_version_id: uuid.UUID, submitted_by: uuid.UUID
    ) -> Execution:
        execution = Execution(
            id=execution_id,
    ...
    def acquire_lease(
        self, execution: Execution, worker_id: uuid.UUID, lease_duration_seconds: int = 300
    ) -> Execution:
        self._assert_not_terminal(execution)

        if execution.status != ExecutionState.QUEUED:
            raise InvalidStateTransitionError(
                f"Cannot acquire lease from state {execution.status.name}"
            )
    ...
    def start_execution(self, execution: Execution, worker_id: uuid.UUID) -> Execution:
        ...
        if execution.status != ExecutionState.SCHEDULED:
            raise InvalidStateTransitionError(
                f"Cannot transition to STARTING from {execution.status.name}"
            )
    ...
    def run_execution(self, execution: Execution, worker_id: uuid.UUID) -> Execution:
        ...
        if execution.status != ExecutionState.STARTING:
            raise InvalidStateTransitionError(
                f"Cannot transition to RUNNING from {execution.status.name}"
            )
    ...
    def complete_cancellation(self, execution: Execution) -> Execution:
        if execution.status != ExecutionState.CANCELLING:
            raise InvalidStateTransitionError(
                "Can only complete cancellation from CANCELLING state."
            )
    ...
    def complete(self, execution: Execution, worker_id: uuid.UUID) -> Execution:
        ...
        if execution.status not in [ExecutionState.RUNNING, ExecutionState.EVALUATING]:
            raise InvalidStateTransitionError(
                f"Cannot complete execution from {execution.status.name}"
            )
        ...
        event = ExecutionCompletedEvent(
            timestamp=self.clock.now(), execution_id=execution.id, attempt_id=attempt.id
        )
        execution.record_event(event)
        return execution

    def fail(
        self, execution: Execution, error_message: str, worker_id: uuid.UUID | None = None
    ) -> Execution:
        # Idempotency check
    ...
    def retry(self, execution: Execution) -> Execution:
        if execution.status != ExecutionState.RETRYING:
            raise InvalidStateTransitionError(
                f"Cannot transition to QUEUED from {execution.status.name}"
            )

        # Re-enforce invariant: Cannot retry if attempt limit is reached
        if (
            execution.current_attempt
            and execution.current_attempt.attempt_number >= execution.max_retries
        ):
