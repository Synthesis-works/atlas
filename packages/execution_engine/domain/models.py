...
class Execution:
    ...
    def begin_attempt(self, worker_id: uuid.UUID, clock: Clock, lease_duration_seconds: int = 300) -> ExecutionAttempt:
        if any(a.status == AttemptStatus.IN_PROGRESS for a in self._attempts):
            raise InvariantViolationError("Exactly one execution attempt is permitted to be in progress.")
    ...
    def is_terminal(self) -> bool:
        return self.status in {ExecutionState.COMPLETED, ExecutionState.FAILED, ExecutionState.CANCELLED}
