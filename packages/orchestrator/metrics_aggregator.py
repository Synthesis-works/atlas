        ...
        for r in results:
            if r.evaluation_status == "pass":
                ...
                if r.execution_status == "success":
                    ...
                    if r.stderr and any(e in r.stderr for e in ["NameError", "ImportError", "ModuleNotFoundError"]):
                        repairable_errors += 1
                    else:
                        unrepairable_errors += 1
            ...
            if not r.extracted_code or r.extracted_code.strip() == "UNKNOWN":
                ...
                if r.raw_response and any(refusal in r.raw_response.lower() for refusal in ["i cannot", "i apologize", "as an ai", "not enough information", "i'm sorry"]):
                    model_refusals += 1
            else:
                prompt_compliance += 1
