...
class RepairStage(PipelineStage):
    ...
    def execute(self, context: dict, state: TaskRunResult) -> TaskRunResult:
        if state.state != TaskRunState.FAILED and getattr(state, "evaluation_status", None) != "fail":
            return state
        ...
        print(f"[{state.task_id}] Attempting automatic repair (Retry {retries+1}/{self.max_retries})...")
