...
class ExperimentRunner:
    ...
    def run(self, exp_config: ExperimentConfig) -> str:
        ...
        print(
            f"Prompt Version: {exp_config.prompt_version}, Tasks: {exp_config.max_tasks or 'All'}"
        )
        ...
        if len(tasks) > 0:
            sample_prompt = PromptBuilder.build_from_task(
                tasks[0],
                version=exp_config.prompt_version,
                benchmark_pack=exp_config.dataset.lower(),
            )
    ...
    def _run_loop(
        ...
        ui = ProgressUI(
            exp_id, exp_config.dataset, exp_config.model, exp_config.prompt_version, total_tasks
        )
    ...
    def _save_checkpoint(self, exp_id: str, ui):
        checkpoint = {
            ...
            "average_latency": (sum(ui.latencies) / len(ui.latencies) / 1000.0)
            if ui.latencies
            else 0.0,
    ...
    def _finalize_experiment(self, exp_id: str, exp_config: ExperimentConfig):
        ...
        registry.register(
            exp_id=exp_id, config=json.loads(exp_config.model_dump_json()), metrics=profile
        )
