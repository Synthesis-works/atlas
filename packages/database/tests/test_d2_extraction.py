...
def setup_d2_fixtures(session):
    ...
    test_project = Project(id=uuid.uuid4(), slug=f"test-proj-{random_suffix}", name=f"Test Project {random_suffix}")
    session.add(test_project)
    session.flush()

    test_dataset = Dataset(id=uuid.uuid4(), project_id=test_project.id, name=f"Test DS {random_suffix}")
    ...
    dv2 = DatasetVersion(id=uuid.uuid4(), dataset_id=test_dataset.id, version_string="v2", storage_path="/", lifecycle="VALID")
...
def test_d2_adversarial_unknown_key(session):
    ...
    assert "SECRET" not in rep, "Adversarial structural leakage detected across Extraction boundary."
    assert "THIS_KEY_MUST_NEVER_LEAK" not in rep, "Metadata blocklist failed closed boundary restriction!"
