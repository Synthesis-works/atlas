...
class EvaluationStrategy(Base, BaseMixin):
    ...
    versions: Mapped[list["EvaluationStrategyVersion"]] = relationship(
        "EvaluationStrategyVersion", back_populates="strategy"
    )
...
class EvaluationStrategyVersion(Base):
    ...
    strategy_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("evaluation_strategies.id"), nullable=False, index=True
    )
    ...
    strategy: Mapped["EvaluationStrategy"] = relationship(
        "EvaluationStrategy", back_populates="versions"
    )
...
class EvaluationResult(Base, BaseMixin):
    ...
    model_output_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("model_outputs.id", ondelete="CASCADE"), nullable=False, unique=True
    )
    strategy_version_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("evaluation_strategy_versions.id"), nullable=False, index=True
    )
    judge_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("judges.id"), nullable=True, index=True
    )
...
class CapabilityScore(Base, BaseMixin):
    ...
    capability_profile_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("capability_profiles.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    profile: Mapped["CapabilityProfile"] = relationship(
        "CapabilityProfile", back_populates="scores"
    )
...
class EvaluationResultDetail(Base):
    ...
    evaluation_result_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("evaluation_results.id", ondelete="CASCADE"), nullable=False, unique=True
    )
    ...
    evaluation_result: Mapped["EvaluationResult"] = relationship(
        "EvaluationResult", back_populates="details"
    )
...
class EvaluationArtifact(Base, BaseMixin):
    ...
    evaluation_result: Mapped["EvaluationResult"] = relationship(
        "EvaluationResult", back_populates="artifacts"
    )
