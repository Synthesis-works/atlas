...
class Organization(Base, BaseMixin):
    ...
    members: Mapped[list["OrganizationMember"]] = relationship("OrganizationMember", back_populates="organization", cascade="all, delete-orphan", foreign_keys="OrganizationMember.organization_id")
    invitations: Mapped[list["Invitation"]] = relationship("Invitation", back_populates="organization", cascade="all, delete-orphan", foreign_keys="Invitation.organization_id")
...
class User(Base, BaseMixin):
    ...
    memberships: Mapped[list["OrganizationMember"]] = relationship("OrganizationMember", back_populates="user", cascade="all, delete-orphan", foreign_keys="OrganizationMember.user_id")
...
class OrganizationMember(Base, BaseMixin):
    ...
    status: Mapped[MembershipStatus] = mapped_column(ENUM(MembershipStatus, name="membership_status"), nullable=False, default=MembershipStatus.ACTIVE)
...
class Invitation(Base, BaseMixin):
    ...
    role: Mapped[OrganizationRole] = mapped_column(ENUM(OrganizationRole, name="organization_role_invitation", create_type=False), nullable=False)
    ...
    organization: Mapped["Organization"] = relationship("Organization", back_populates="invitations", foreign_keys=[organization_id])
...
class Project(Base, BaseMixin):
    ...
    updated_by_member_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("organization_members.id", ondelete="SET NULL"), nullable=True)
...
class Configuration(Base, BaseMixin):
    ...
    scope: Mapped[ConfigurationScope] = mapped_column(
        ENUM(ConfigurationScope, name="configuration_scope"),
        nullable=False
    )
    project_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("projects.id", ondelete="CASCADE"), nullable=True, index=True)
    benchmark_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("benchmarks.id", ondelete="CASCADE"), nullable=True, index=True)
...
class ConfigurationVersion(Base):
    ...
    configuration_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("configurations.id"), nullable=False, index=True)
