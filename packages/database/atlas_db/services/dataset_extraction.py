    ...
    def get_training_examples(self, dataset_version_id: uuid.UUID) -> list[TrainingExample]:
        ...
        for task in tasks:
            ...
            if len(task.prompts) == 0:
                raise ValueError(f"Ambiguity/Missing Error: Task {task.id} has no attached Prompts.")
            if len(task.prompts) > 1:
                raise ValueError(f"Ambiguity Error: Task {task.id} possesses multiple Prompts; extraction unsafe.")
            ...
            if len(public_cases) == 0:
                raise ValueError(f"Ambiguity/Missing Error: Task {task.id} has no public TestCase serving as canonical answer.")
            if len(public_cases) > 1:
                raise ValueError(f"Ambiguity Error: Task {task.id} possesses multiple public TestCases; extraction unsafe.")
            ...
            metadata = {
                ...
                if k in ALLOWED_TRAINING_METADATA_KEYS
            }
