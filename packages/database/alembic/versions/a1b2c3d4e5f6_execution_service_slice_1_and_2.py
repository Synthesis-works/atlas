...
down_revision: Union[str, Sequence[str], None] = '27624aa4c089'
...
def upgrade() -> None:
    ...
    worker_status_enum = postgresql.ENUM('REGISTERED', 'READY', 'BUSY', 'OFFLINE', name='worker_status')
    worker_status_enum.create(op.get_bind())

    event_type_enum = postgresql.ENUM(
        'RUN_CREATED', 'RUN_VALIDATED', 'RUN_QUEUED', 'RUN_STARTED', 'RUN_PAUSED', 'RUN_RESUMED',
        'RUN_FAILED', 'RUN_COMPLETED', 'RUN_CANCELLED', 'TASK_QUEUED', 'TASK_ASSIGNED', 'TASK_STARTED',
        'TASK_COMPLETED', 'TASK_FAILED', 'WORKER_HEARTBEAT', 'WORKER_REGISTERED', 'WORKER_OFFLINE',
        name='event_type'
    )
    ...
    op.create_index(op.f('ix_execution_workers_adapter_id'), 'execution_workers', ['adapter_id'], unique=False)
    ...
    op.create_table('atlas_tasks',
        ...
        sa.ForeignKeyConstraint(['assigned_worker_id'], ['execution_workers.id'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_atlas_tasks_atlas_run_id'), 'atlas_tasks', ['atlas_run_id'], unique=False)
    op.create_index(op.f('ix_atlas_tasks_assigned_worker_id'), 'atlas_tasks', ['assigned_worker_id'], unique=False)
    ...
    op.create_table('run_events',
        ...
        sa.ForeignKeyConstraint(['execution_worker_id'], ['execution_workers.id'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_run_events_atlas_run_id'), 'run_events', ['atlas_run_id'], unique=False)
    op.create_index(op.f('ix_run_events_atlas_task_id'), 'run_events', ['atlas_task_id'], unique=False)
    op.create_index(op.f('ix_run_events_execution_worker_id'), 'run_events', ['execution_worker_id'], unique=False)
    op.create_index(op.f('ix_run_events_type'), 'run_events', ['type'], unique=False)

    # 6. Alter atlas_runs table
    op.add_column('atlas_runs', sa.Column('last_heartbeat_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('atlas_runs', sa.Column('config', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
