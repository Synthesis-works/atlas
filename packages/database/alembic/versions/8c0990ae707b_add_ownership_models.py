...
def upgrade():
    # Enums
    organization_role = postgresql.ENUM('OWNER', 'ADMIN', 'MEMBER', 'VIEWER', name='organization_role')
    organization_role.create(op.get_bind(), checkfirst=True)
    
    membership_status = postgresql.ENUM('ACTIVE', 'PENDING', 'SUSPENDED', 'LEFT', name='membership_status')
    membership_status.create(op.get_bind(), checkfirst=True)
    
    invitation_status = postgresql.ENUM('PENDING', 'ACCEPTED', 'EXPIRED', 'REVOKED', name='invitation_status')
