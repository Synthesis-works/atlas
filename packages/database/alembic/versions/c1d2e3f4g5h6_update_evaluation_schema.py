...
def upgrade() -> None:
    ...
    evaluation_status = postgresql.ENUM('completed', 'partial_success', 'failed', name='evaluation_status')
    evaluation_status.create(op.get_bind())

    # 2. Add columns to evaluation_results
    op.add_column('evaluation_results', sa.Column('status', evaluation_status, nullable=False, server_default='completed'))
    op.add_column('evaluation_results', sa.Column('evaluation_context', postgresql.JSONB(astext_type=sa.Text()), nullable=True))

    # 3. Add column to capability_profiles
    op.add_column('capability_profiles', sa.Column('score_explanation', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    ...
    op.create_index(op.f('ix_evaluation_artifacts_evaluation_result_id'), 'evaluation_artifacts', ['evaluation_result_id'], unique=False)

def downgrade() -> None:
    # Drop evaluation_artifacts table
    op.drop_index(op.f('ix_evaluation_artifacts_evaluation_result_id'), table_name='evaluation_artifacts')
