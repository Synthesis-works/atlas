...
def upgrade() -> None:
    ...
    with op.batch_alter_table("executions", schema=None) as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_executions_dataset_version_id"), ["dataset_version_id"], unique=False
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_executions_dataset_version_id_dataset_versions"),
            "dataset_versions",
            ["dataset_version_id"],
            ["id"],
            ondelete="CASCADE",
    ...
    with op.batch_alter_table("test_cases", schema=None) as batch_op:
        batch_op.add_column(sa.Column("dataset_version_id", sa.Uuid(), nullable=True))
        batch_op.create_index(
            batch_op.f("ix_test_cases_dataset_version_id"), ["dataset_version_id"], unique=False
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_test_cases_dataset_version_id_dataset_versions"),
            "dataset_versions",
            ["dataset_version_id"],
            ["id"],
            ondelete="CASCADE",
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("test_cases", schema=None) as batch_op:
        batch_op.drop_constraint(
            batch_op.f("fk_test_cases_dataset_version_id_dataset_versions"), type_="foreignkey"
        )
    ...
    with op.batch_alter_table("executions", schema=None) as batch_op:
        batch_op.drop_constraint(
            batch_op.f("fk_executions_dataset_version_id_dataset_versions"), type_="foreignkey"
        )
