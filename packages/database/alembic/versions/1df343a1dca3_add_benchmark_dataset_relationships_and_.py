...
down_revision: str | Sequence[str] | None = "a5eef4ee78a5"
...
def upgrade() -> None:
    with op.batch_alter_table("benchmark_versions", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("metric_config", postgresql.JSONB(astext_type=sa.Text()), nullable=True)
        )
        batch_op.add_column(
            sa.Column("scoring_policy", postgresql.JSONB(astext_type=sa.Text()), nullable=True)
        )
        batch_op.create_index(
            batch_op.f("ix_benchmark_versions_primary_dataset_version_id"),
            ["primary_dataset_version_id"],
            unique=False,
        )
        batch_op.create_foreign_key(
            batch_op.f("fk_benchmark_versions_primary_dataset_version_id_dataset_versions"),
            "dataset_versions",
            ["primary_dataset_version_id"],
            ["id"],
            ondelete="RESTRICT",
