...
def upgrade() -> None:
    ...
    if bind.engine.name == 'sqlite':
        with op.batch_alter_table('capability_profiles') as batch_op:
            ...
            batch_op.create_foreign_key(None, 'evaluation_results', ['evaluation_id'], ['id'], ondelete='CASCADE')
            batch_op.create_foreign_key(None, 'evaluation_strategy_versions', ['strategy_version_id'], ['id'])
        
        op.create_index(op.f('ix_capability_profiles_evaluation_id'), 'capability_profiles', ['evaluation_id'], unique=False)
        op.create_index(op.f('ix_capability_profiles_execution_id'), 'capability_profiles', ['execution_id'], unique=False)
        op.create_index(op.f('ix_capability_profiles_strategy_version_id'), 'capability_profiles', ['strategy_version_id'], unique=False)
    else:
        op.drop_constraint('uq_capability_profiles_atlas_run_id', 'capability_profiles', type_='unique')
        ...
        op.add_column('capability_profiles', sa.Column('strategy_version_id', sa.Uuid(), nullable=True))
        op.add_column('capability_profiles', sa.Column('profile_version', sa.Integer(), nullable=True))
        
        # 3. Create foreign keys and indexes
        op.create_foreign_key(None, 'capability_profiles', 'evaluation_results', ['evaluation_id'], ['id'], ondelete='CASCADE')
        op.create_foreign_key(None, 'capability_profiles', 'evaluation_strategy_versions', ['strategy_version_id'], ['id'])
...
def downgrade() -> None:
    op.drop_index(op.f('ix_capability_profiles_strategy_version_id'), table_name='capability_profiles')
    ...
    op.create_unique_constraint('uq_capability_profiles_atlas_run_id', 'capability_profiles', ['atlas_run_id'])
