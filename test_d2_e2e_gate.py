...
def test_humaneval_e2e(engine):
    ...
    manifest = DatasetManifest(
        id="he-pack", name="HE Pack", version="1.0", source="test",
        license="test", citation="test", language="python",
        evaluation="execution", metric="pass@1", tags=[], tasks=1
    )
    ...
    with Session(engine) as session:
        ...
        test_dv = DatasetVersion(dataset_id=t_ds.id, version_string="v1", storage_path="/", lifecycle="VALID")
        session.add(test_dv)
        session.flush()
    ...
    with Session(engine) as session:
        ...
        assert raw["test"] not in ex.prompt and raw["test"] not in ex.canonical_answer, "Test hidden"
...
def test_mbpp_e2e(engine):
    ...
    manifest = DatasetManifest(
        id="mb-pack", name="MB Pack", version="1.0", source="test",
        license="test", citation="test", language="python",
        evaluation="execution", metric="pass@1", tags=[], tasks=1
    )
    pack = DatasetPack(manifest=manifest, tasks=[task])
