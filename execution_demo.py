...
def demo_happy_path(db_session):
    ...
    # 1. Register Worker
    worker_id = worker_ctrl.execute_register_worker(
        RegisterWorkerCommand(
    ...
    task_ids = []
    for _ in range(3):
        t = AtlasTask(atlas_run_id=run_id, test_case_id=uuid.uuid4(), status=TaskStatus.PENDING)
        db_session.add(t)
        db_session.flush()
    ...
    for i, t in enumerate(running_tasks, 1):
        ...
        task_ctrl.execute_complete_task(
            CompleteTaskCommand(worker_id=worker_id, task_id=t.id, raw_output=f"mock_answer_{i}")
        )
...
def demo_ownership_mismatch(db_session):
    ...
    try:
        task_ctrl.execute_complete_task(
            CompleteTaskCommand(worker_id=worker_b, task_id=task_id, raw_output="hacked")
        )
